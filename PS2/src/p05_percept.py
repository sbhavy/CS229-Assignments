...
def predict(state, kernel, x_i):
    ...
    # *** START CODE HERE ***
    ans = 0
    for x, beta_i in zip(state['support_vec'], state['beta']):
        ans += beta_i * kernel(x_i, x)
    return ans
    # *** END CODE HERE ***
...
def update_state(state, kernel, learning_rate, x_i, y_i):
    ...
    """
    # *** START CODE HERE ***
    beta_i = learning_rate * (y_i - sign(predict(state, kernel, x_i)))
    state['support_vec'].append(x_i)
