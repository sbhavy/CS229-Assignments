...
def backward_convolution(conv_W, conv_b, data, output_grad):
    ...
    # *** START CODE HERE ***
    G_bias = output_grad.sum(axis=(1,2))

    conv_channels, _, conv_width, conv_height = conv_W.shape
    _, W, H = data.shape

    G_weight, G_data = np.zeros(conv_W.shape), np.zeros(data.shape)

    for i in range(W - conv_width + 1):
        for j in range(H - conv_height + 1):
            for output_channel in range(conv_channels):
                G_weight[output_channel, :, :, :] += output_grad[output_channel, i, j] * data[:, i:(i + conv_width), j:(j + conv_height)]
                G_data[:, j:(j + conv_width), j:(j + conv_height)] += output_grad[output_channel, i, j] * conv_W[output_channel, :, :, :]

    return (G_weight, G_bias, G_data)
    # *** END CODE HERE ***
...
def backward_max_pool(data, pool_width, pool_height, output_grad):
    ...
    for i in range(C):
        for j in range(0, W, pool_width):
            for k in range(0, H, pool_height):
                t = data[i, j:(j + pool_width), k:(k + pool_height)]
                grads[i, j:(j + pool_width), k:(k + pool_height)][i, np.unravel_index(t.argmax(), t.shape)] = output_grad[i, j // pool_width, k // pool_height]
...
def backward_linear(weights, bias, data, output_grad):
    ...
    # *** START CODE HERE ***
    G_weight = np.outer(data, output_grad)
    G_bias = output_grad
    G_data = weights.dot(output_grad)
...
def backward_prop(data, labels, params):
    ...
    first_conv = forward_convolution(W1, b1, data)
    first_max_pool = forward_max_pool(first_conv, MAX_POOL_SIZE, MAX_POOL_SIZE)
    first_relu = forward_relu(first_max_pool)
    flattened = np.reshape(first_relu, (-1))
    logits = forward_linear(W2, b2, flattened)
    y = forward_softmax(logits)

    G_loss = backward_cross_entropy_loss(y, labels)
    G_softmax = backward_softmax(logits, G_loss)
    G_W2, G_b2, G_linear = backward_linear(W2, b2, flattened, G_softmax)
    G_relu = backward_relu(first_max_pool, G_linear.reshape(first_max_pool.shape))
    G_max_pool = backward_max_pool(first_conv, MAX_POOL_SIZE, MAX_POOL_SIZE, G_relu)
    G_W1, G_b1, _ = backward_convolution(W1, b1, data, G_max_pool)
